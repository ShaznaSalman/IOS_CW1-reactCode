...
const HeartBtn = ({ filled = false }: { filled?: boolean }) => (
  <div
    onClick={e => e.stopPropagation()} // prevent card click when tapping heart
    style={{ width: 32, height: 32, borderRadius: "50%", backgroundColor: filled ? "#FFEBEE" : "#fff", border: "1px solid #eee", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}
  >
...
// ── Doctor card ───────────────────────────────────────────────────────────────
const DoctorCard = ({ name, specialty, img, favored = false, slug, onNavigate }: {
  name: string; specialty: string; img: string; favored?: boolean; slug: string; onNavigate: (slug: string) => void;
}) => (
  <div
    onClick={() => onNavigate(slug)}
    style={{ backgroundColor: "#fff", borderRadius: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.07)", padding: 14, marginBottom: 12, cursor: "pointer" }}
  >
    ...
    <button
      ...
      style={{ width: "100%", marginTop: 12, padding: "11px 0", borderRadius: 10, backgroundColor: "#EAF5FE", border: "none", color: "#2196F3", fontSize: 14, fontWeight: "700", cursor: "pointer", fontFamily: "inherit", letterSpacing: -0.1 }}
    >
      Make Appointment
    </button>
  </div>
...
// ── Lab / Pharmacy image card ─────────────────────────────────────────────────
const ImageCard = ({ name, category, location, hours, open, img, slug, onNavigate }: {
  name: string; category: string; location: string; hours: string; open: boolean; img: string; slug: string; onNavigate: (slug: string) => void;
}) => (
  <div
    onClick={() => onNavigate(slug)}
    style={{ backgroundColor: "#fff", borderRadius: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.07)", overflow: "hidden", marginBottom: 12, cursor: "pointer" }}
  >
...
export default function ServicesScreen({ initialTab = "Doctor" }: ServicesScreenProps) {
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState<Tab>(initialTab);

  // Tab switching also updates the URL
  const handleTabChange = (tab: Tab) => {
    setActiveTab(tab);
    navigate(`/services/${tab.toLowerCase()}`, { replace: true });
  };

  const goToDoctor   = (slug: string) => navigate(`/services/doctor/${slug}`);
  const goToLab      = (slug: string) => navigate(`/services/lab/${slug}`);
  const goToPharmacy = (slug: string) => navigate(`/services/pharmacy/${slug}`);
    ...
    <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "100vh", backgroundColor: "#111" }}>
      <PhoneShell>
        ...
        <div style={{ flex: 1, overflowY: "auto", backgroundColor: "#F4F6FB", padding: "16px 18px", scrollbarWidth: "none" }}>
          ...
          {activeTab === "Doctor" && (
            <>
              <DoctorCard name="Dr. Jayaani Dennis" specialty="Physiologist" img={IMG.doc1} favored slug="jayaani-dennis"  onNavigate={goToDoctor} />
              <DoctorCard name="Dr. Ryan De Silva"   specialty="Cardiologist" img={IMG.doc2}        slug="ryan-de-silva"   onNavigate={goToDoctor} />
              <DoctorCard name="Dr. Sarath Fernando" specialty="Dentist"      img={IMG.doc3}        slug="sarath-fernando" onNavigate={goToDoctor} />
            </>
          )}

          {activeTab === "Lab" && (
            <>
              <ImageCard name="X - Ray"    category="Radiology, Skeletal"       location="Ground Floor, B - Wing" hours="Mon - Sat | 10:00 AM - 08:00 PM" open    img={IMG.xray}   slug="xray"       onNavigate={goToLab} />
              <ImageCard name="CT Scan"    category="Radiology, Internal Organs" location="Ground Floor, A - Wing" hours="Mon - Sat | 11:00 AM - 05:00 PM" open={false} img={IMG.ctScan} slug="ct-scan"     onNavigate={goToLab} />
              <ImageCard name="Blood Test" category="Hematology, General"        location="Level 2, C - Wing"      hours="Mon - Sun | 07:00 AM - 09:00 PM" open    img={IMG.blood}  slug="blood-test"  onNavigate={goToLab} />
            </>
          )}

          {activeTab === "Pharmacy" && (
            <>
              <ImageCard name="Clinic Flow Pharmacy" category="Internal Drugstore" location="Aura Building, Ground Floor" hours="Mon - Sun | 08:00 AM - 11:00 PM" open img={IMG.pharmacy1} slug="clinic-flow-pharmacy" onNavigate={goToPharmacy} />
              <ImageCard name="MedPlus Pharmacy"     category="Retail Pharmacy"    location="Block B, Level 1"           hours="Mon - Sat | 09:00 AM - 10:00 PM" open img={IMG.pharmacy2} slug="medplus-pharmacy"    onNavigate={goToPharmacy} />
            </>
          )}
